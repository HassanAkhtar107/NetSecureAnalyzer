...
const Received = () => {
  ...
  const getFileIcon = (type) => {
    switch (type?.toLowerCase()) {
      ...
      case 'document': return <FileText className="text-sky-400" size={20} />;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          receivedFiles.map((file, idx) => (
            <div 
              key={file.id || idx}
              className="glass-panel p-6 space-y-4 hover:border-emerald-500/30 transition-all group relative overflow-hidden animate-fade-in"
            >
